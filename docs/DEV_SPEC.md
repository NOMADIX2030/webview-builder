...
| 계층 | 기술 | 버전 | 역할 |
|------|------|------|------|
| **프론트엔드** | Laravel Blade + Tailwind | - | 3단계 폼 UI, 시뮬레이션 (서버사이드 렌더링) |
...
| **Blade** | 단일 앱 구조, 디버깅 용이, 서버사이드 렌더링 |
...
#### 2.3.1 Blade + Tailwind 규칙

| 규칙 | 내용 |
|------|------|
| **템플릿** | Blade 문법, `@extends`, `@section`, `@yield` |
| **스타일** | Tailwind CSS |
| **폼** | Laravel `$request->validate()`, `@error` |
...
├── webview-builder/                          # Laravel 앱 (Blade UI + API + 빌드)
│   ├── app/
│   │   ├── Http/Controllers/
...
| 경로 | 처리 | 접근 |
|------|------|------|
| `/` | Laravel Blade (webview-builder/public) | http://localhost/ |
| `/api` | Laravel API (webview-builder/public) | http://localhost/api |
| `/storage` | 업로드 파일 | http://localhost/storage |

**시작 순서**: nginx, PHP-FPM (별도 프론트엔드 서버 없음)
...
```json
{
  ...
  "artifacts": {
    "apk": "http://localhost/api/build/xxx/download/apk",
    "ipa": "http://localhost/api/build/xxx/download/ipa",
    "keystore": "http://localhost/api/build/xxx/download/keystore"
  },
...
**Response**: 파일 스트림 (Content-Disposition: attachment, Content-Length, Content-Type: application/vnd.android.package-archive for APK)

**참고**: Blade에서 `artifacts.apk` 등 전체 URL 사용. X-Accel-Redirect로 nginx가 직접 파일 서빙 (APK 잘림 방지).
...
> **상세**: 테이블 구축 방법, 마이그레이션 절차는 **DATABASE.md** 참조.
...
| 단계 | 경로 | 이전 | 다음 |
|------|------|------|------|
| 1 | `/` 또는 `/build/step1` | - | `/build/step2` |
| 2 | `/build/step2` | `/build/step1` | `/build/step3` |
| 3 | `/build/step3` | `/build/step2` | `/build/{id}` |
| 결과 | `/build/{id}` | `/build/step3` | - |

### 9.3 모바일 시뮬레이터 (3단계)

- **Blade + Tailwind**로 모바일 프레임 구현
- iPhone/Android 프레임: CSS 또는 Tailwind 스타일로 구현
...
| 순서 | 작업 | 명령/방법 | 완료 조건 |
|------|------|-----------|-----------|
| 1-1 | 프로젝트 루트 확인 | 워크스페이스: `/Users/awekers/Sites/www` | `webview-builder/`, `docs/`, `nginx/` 폴더 구조 준비 |
| 1-2 | Laravel 프로젝트 생성 | `webview-builder/` 폴더에 `composer create-project laravel/laravel .` | Laravel 앱 동작 확인 |
| 1-2a | Laravel DB 설정 | `webview-builder/.env`에 10.10 환경 변수 반영. `webview-builder.env.example` 참조. DB_DATABASE=webview_builder | `php artisan migrate` 성공 |
| 1-3 | Blade + Tailwind | `webview-builder/resources/views/`, `npm run build` (Vite) | Blade 페이지 동작 확인 |

---

### 10.2 Phase 2: (현재 Blade + Tailwind 적용됨)

> **참고**: Next.js/shadcn/ui 제거됨. Laravel Blade + Tailwind 사용.
...
| 순서 | 작업 | 내용 | 완료 조건 |
|------|------|------|-----------|
| 3-1 | 마이그레이션 생성 | `builds` 테이블. **DATABASE.md** 3.4 예시 참조 | `php artisan migrate` 성공 |
...
### 10.4 Phase 4: Blade UI 구축

| 순서 | 작업 | 경로 | 완료 조건 |
|------|------|------|-----------|
| 4-1 | 1단계 페이지 | `resources/views/step1.blade.php` | 도메인, 로고 입력, 2단계 이동 |
| 4-2 | 2단계 페이지 | `resources/views/step2.blade.php` | 자동 생성값 표시·수정, 3단계 이동 |
| 4-3 | 3단계 페이지 | `resources/views/step3.blade.php` | 시뮬레이션, 최종 확인, 빌드 요청 |
| 4-4 | 빌드 결과 페이지 | `resources/views/build/show.blade.php` | 상태 폴링, 다운로드 링크 |
| 4-5 | Web BuildController | `app/Http/Controllers/Web/BuildController.php` | 세션 기반 폼 처리 |
| 4-6 | 모바일 우선 스타일 | 모든 페이지 | 375px~430px 기준 레이아웃 |

**UI 규칙**: Blade + Tailwind CSS.

---

### 10.5 Phase 5: Capacitor 템플릿 및 빌드

> **빌드 환경**: APK 빌드 시 Java JDK 17, Android SDK 필요. → **BUILD_ENVIRONMENT.md** 참조.

| 순서 | 작업 | 내용 | 완료 조건 |
|------|------|------|-----------|
| 5-1 | Capacitor 템플릿 생성 | `webview-builder/storage/app/build-templates/webview-app/` | `www/`, `android/`, `capacitor.config.json` |
...
```
[ ] Phase 1: Laravel 초기화
[ ] Phase 2: Blade + Tailwind (Vite 빌드)
[ ] Phase 3: Laravel API, Job, Service 구현
[ ] Phase 4: Blade 페이지 (1→2→3→build) 구현
...
| **학습 소스** | Tailwind CSS, Laravel Blade, Material Design 가이드라인 참고 |

**참조 URL**: https://tailwindcss.com | https://laravel.com/docs/blade
...
| **프로젝트 현황** | PROJECT_STATUS.md (완료 기능, 다음 단계) |
| 기술 스택 아키텍처 | TECH_STACK.md |
| AI 협업 프로세스 | AI_COLLABORATION_PROCESS.md |
| **데이터베이스** | DATABASE.md (테이블 구축, 마이그레이션) |
| **환경 설정 방안** | ENVIRONMENT_SETUP.md (nginx, 개발 실행 방식) |
| **APK 빌드 환경** | BUILD_ENVIRONMENT.md (Java, Android SDK 설치) |
| 백엔드 환경 변수 템플릿 | webview-builder.env.example |
...
| 2026-02-27 | 2.3 디자인 표준 추가 (Blade + Tailwind, 모바일 우선, 네이티브 앱 수준) |
| 2026-02-27 | 10. 필수 업무 진행 방법 추가 (Phase 1~5, AI 체크리스트) |
