...
class BuildController extends Controller
{
     ...
     */
    public function download(string $buildId, string $type): \Illuminate\Http\Response|JsonResponse
    {
        $build = Build::find($buildId);
        ...
        if (! file_exists($fullPath)) {
            Log::warning('다운로드 파일 없음', ['build_id' => $buildId, 'path' => $fullPath]);
            return response()->json(['error' => '파일을 찾을 수 없습니다.'], 404);
        }

        $filename = basename($path);
        Log::info('다운로드 시작', ['build_id' => $buildId, 'type' => $type]);
        ...
        // X-Accel-Redirect: nginx가 직접 파일 서빙 → PHP 버퍼/nginx fastcgi 버퍼 우회
        $internalPath = '/internal-download/' . $path;
