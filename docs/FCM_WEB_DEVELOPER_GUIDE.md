...
| **알림 배지 실시간 갱신** | FCM data 수신 시 `window.__onPushReceived(data)` 호출. 포그라운드에서 헤더 배지 등 즉시 갱신 가능 (섹션 3.8 참조) |
...
### 3.8 알림 배지 실시간 갱신 (__onPushReceived)

앱이 **포그라운드**일 때 FCM data 메시지를 수신하면, 앱이 WebView에 `window.__onPushReceived(data)`를 호출합니다. 웹에서 이 콜백을 정의하면 헤더 종 아이콘의 배지 숫자 등을 **폴링 없이** 실시간으로 갱신할 수 있습니다.

**동작 조건**

- 앱이 포그라운드(화면에 보이는 상태)일 때만 호출됨
- `window.__onPushReceived`가 정의되어 있을 때만 호출 (없으면 무시, 에러 없음)
- 백그라운드 수신 시에는 호출되지 않음. 포그라운드 복귀 시 웹이 새로 로드되므로 별도 처리 불필요

**서버 FCM data 예시 (unread_count 포함)**
...
```javascript
window.__onPushReceived = function(data) {
  if (!data) return;
  var count = parseInt(data.unread_count || '0', 10);
  // 헤더 배지 등 UI 갱신
  document.getElementById('badge-count').textContent = count > 99 ? '99+' : count;
};
```

- `data`: `{ unread_count, title, body, type }` 등 FCM data 필드
- Opt-in: 정의하지 않으면 아무 동작 없음 (다양한 웹사이트에 안전)
